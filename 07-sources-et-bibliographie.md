...
| Sondes et extensions mesurées sur machine | `SC62015Disassembler/Samples/DEVICE9/` et `Samples/BASEXT/` | `SQR`, `ADDTEST`, `DIVTEST` (device 9, famille 0), `MATTEST` (famille 1, matrices, 2026-09-14), module `BASEXT`. Chaque source consigne en en-tête le résultat observé et son interprétation — source de `04` §2.5 et de `12`. |
...
- **[Takayuki Mizuno — kt.rim.or.jp/~tmizuno](http://www.kt.rim.or.jp/~tmizuno/pocket/library/sharp01.html)** — index de bibliothèque logicielle SHARP incluant XASM/DISASM12 (miroir des fichiers de Kako), l'assembleur alternatif **A62**/**MASSE** (Nmasu), le compilateur **VOGUE** (契約 稔 / N. Kon — le même « N. Kon » crédité comme co-auteur de XASM) — ⚠️ *à confirmer : `契約 稔` a l'allure d'un artefact d'encodage (§6), et `SC62015Disassembler/CLAUDE.md` attribue le `vogue` de son corpus de test à **J.-F. Albouy** ; il peut s'agir de deux programmes distincts du même nom*, et **COMPO-System**/**SG** (TORO / 高橋 良和).
