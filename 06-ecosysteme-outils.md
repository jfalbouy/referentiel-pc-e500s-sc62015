...
| **`BASEXT`** (`C:\Claude\BASEXT`) | **Projet transverse**, et **référent** pour la création d'instructions BASIC : le module de douze mots-clés (`LPEEK`, `WPEEK`, `LPOKE`, `MOD`, `UCASE$`, `LCASE$`, `TRIM$`, `LTRIM$`, `RTRIM$`, `REPT$`, `SREPT$`, `INSTR`), le `MODE-EMPLOI.md` (168 instructions standard lues dans l'image, procédure, adresses ROM, restitution), les essais et l'extracteur de tokens. Anciennement `SC62015Disassembler/Samples/BASEXT`. | Validé sur émulateur PC-E500S le 2026-09-15. | Source `.ASM` → `.OBJ` / `.UU` + `.BAS` d'essai ; `rom83.bin` → table des tokens. |
