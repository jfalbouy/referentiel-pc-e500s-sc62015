...
| **`xasm2026-4`** | Réécriture C# (.NET 8) du même assembleur. | Fonctionnel, non-régression **bit-exacte** vs `xasm2026-1`. | Même CLI/formats, implémentation native C#. |
| **`SC62015Disassembler`** (+ mirroir `-github`) | Désassembleur C#/.NET 9, lit les objets produits par XASM (tous formats, + images ROM). | **Complet** : 249 tests, reproduction 100 % des listings XASM de référence (`tmap` 553/553, `vogue` 5455/5455 instructions). | `.obj`/ROM → `.lst` (listing annoté) / `.asm` (source réassemblable) / `.json`/`.csv`. |
